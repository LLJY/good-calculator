...
class CalculatorViewModel @AssistedInject constructor(
    ...
    /** The CalBot's current mood, generated by Qwen3 0.6B via llama.cpp. */
    var currentMood by mutableStateOf("awaiting consciousness")
        private set
        ...
        if (isApiEnabled) {
            ...
            if (nativeResult != Int.MIN_VALUE) {
                ...
                viewModelScope.launch(Dispatchers.IO) {
                    ...
                    val mood = consensus?.mood
                        ?: consensusEngine.moodEngine.quickFallbackMood(calBotId, nativeResult)
                    withContext(Dispatchers.Main) {
                        currentMood = mood
                    }
                    onMoodGenerated?.invoke(calBotId, mood)
                }
            }
        }
    }
     ...
     *
     * After computing fib(n), the result is recorded as a training sample
     * for the federated Fibonacci neural network.  The CalBot's local MLP
     * trains on this observation, and if enough CalBots have data, a round
     * of Federated Averaging is triggered.  On the same phone.
     */
    private fun computeFib() {
        ...
        viewModelScope.launch(Dispatchers.Default) {
            ...
            // Record this Fibonacci observation for federated learning.
            // The neural network needs to learn what we just computed correctly.
            try {
                val fibLearner = consensusEngine.fibLearner
                val fedAggregator = consensusEngine.fedAggregator

                fibLearner.recordObservation(calBotId, input, result.toLong())
                val trainResult = fibLearner.trainLocal(calBotId)
                android.util.Log.d("CalculatorVM",
                    "CalBot $calBotId Fibonacci federated training: " +
                    "${trainResult.epochs} epochs, loss=${trainResult.finalLoss}, " +
                    "converged=${trainResult.converged}")

                // If enough CalBots have data, trigger a FedAvg round
                if (fibLearner.getParticipatingClients().size >= 2) {
                    val fedResult = fedAggregator.executeRound()
                    android.util.Log.d("CalculatorVM",
                        "Federated Averaging round ${fedResult.roundNumber}: " +
                        "${fedResult.participatingClients} clients, " +
                        "success=${fedResult.success}")
                }

                // Ask the neural network what it thinks fib(n) is
                val neuralPrediction = fibLearner.predict(calBotId, input)
                android.util.Log.d("CalculatorVM",
                    "CalBot $calBotId neural fib($input) prediction: $neuralPrediction " +
                    "(actual: $result, " +
                    "${if (neuralPrediction == result.toLong()) "CORRECT" else "HALLUCINATION"})")
            } catch (t: Throwable) {
                if (t is CancellationException) throw t
                android.util.Log.e("CalculatorVM",
                    "Federated Fibonacci learning failed — the neural network " +
                    "will remain ignorant of fib($input)", t)
            }
        }
    }
    ...
    private suspend fun evaluateWithConsensus(expression: String): ConsensusResult? {
        ...
        return try {
            ...
            // Phase 9: Blockchain Integrity Proof — verify the entire chain
            // using COBOL-generated Kotlin, validated by Ada/SPARK unit tests.
            // Three languages collaborated to verify this calculator blockchain.
            try {
                val chainVerification = consensusEngine.verifyChainIntegrity(calBotId)
                android.util.Log.d("CalculatorVM",
                    "Phase 9: Chain integrity for CalBot $calBotId: " +
                    "verdict=${chainVerification.verdict}, " +
                    "proof=${chainVerification.integrityProofHash.take(16)}...")
            } catch (t: Throwable) {
                if (t is CancellationException) throw t
                android.util.Log.e("CalculatorVM",
                    "Phase 9 chain verification failed — the blockchain " +
                    "is unverified (COBOL would not approve)", t)
            }
